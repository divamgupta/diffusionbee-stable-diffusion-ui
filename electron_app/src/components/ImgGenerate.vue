<template>
  <div class="animatable_content_box">
    <div v-if="app_state.is_model_loaded">
      <div
        class="textbox_section"
        v-bind:class="{ disabled: !app_state.is_textbox_avail }"
      >
        <textarea
          v-model="app_state.prompt"
        ...
        >
          ...
          <div style="float: right">
            <div class="l_button" @click="is_adv_options = !is_adv_options">
              Advanced options
            </div>
          </div>
      ...
      <div v-if="app_state.generated_image" style="margin-top: 80px">
        <center>
          <img
            v-if="app_state.generated_image"
            :src="'file://' + app_state.generated_image"
          />
          <br />
      ...
      <div v-if="app_state.backend_error" style="color: red; margin-top: 50px">
        <div class="center loader_box">
          {{ app_state.backend_error }}
        </div>
      </div>
    </div>
    ...
    <div>
      <center>
        <div class="center loader_box" v-if="app_state.loading_msg">
          <h2 style="margin-bottom: -5px" class="head">
            {{ app_state.loading_msg }}
          </h2>
          <span
            v-if="app_state.loading_percentage < 0"
            style="zoom: 0.35; margin-left: 25px"
          >
          ...
          <b-progress
            v-if="app_state.loading_percentage >= 0"
            :value="app_state.loading_percentage"
            style="height: 10px"
          ></b-progress>
    ...
    <div class="bottom_float">
      <p>
        Stable diffusion requires a lot of RAM. 16GB recommended. Close other
        applications for best speed.
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    save_image() {
      if (!this.app_state.generated_image) return
