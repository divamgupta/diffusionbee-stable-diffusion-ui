<template>
    <div v-click-outside="unclick_card_body" class="responsive_gallery_container " v-bind:class="{ 'px500_less_size_gallery' :  is_px500_less_size_gallery , 'is_full_size' : is_full_size }">
        <div v-on:click="click_card_body() ; $emit('on_click_cb', item_id) "  class="gallery_card" v-bind:class="{ 'selected' : selected }">

             

            <video class="gallery_img" v-if="type=='full_img' && video_url && is_full_size" width="100%" :src="video_url"  controls disablePictureInPicture> </video>   
            ...
            <div v-if="type == 'text' "  >
                <div class = "paragraph_container" style="padding:16px; overflow: hidden ">
                    <div class="paragraph_container_inner">
                        <p style=" font-size: 16px ; line-height: 22px ; font-weight: normal ; color:rgba(0, 0, 0, 0.6);">{{description}} </p>
                    </div>
...
export default {
    ...
    methods: {
        ...
        unclick_card_body: function() {
            if(this.dont_unselect_on_outside_click)
                return;
            else
                this.selected = false
        },
        ...
        click_card_body: function() {
            ...
            const thatt = this
            setTimeout(function() { thatt.just_selected = false }, 200)
