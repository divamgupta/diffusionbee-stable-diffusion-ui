<template>
  <div>
    <FormInput
      v-for="form_el in form_entries_to_show"
      ...
      :constraints="form_el.constraints"
...
export default {
  ...
  methods: {
    is_render_condition_satisfied(render_condition) {
      if (
        render_condition['comparator'] == 'eq' &&
        this.form_values[render_condition['lhs']] == render_condition['rhs']
      )
        return true
      else return false
    },
    are_form_entry_conditions_satisfied(render_conditions) {
      for (let render_condition of render_conditions) {
        if (!this.is_render_condition_satisfied(render_condition)) return false
      }
      return true
    },
  },
    ...
    form_entries_to_show() {
      ...
      for (let form_el of this.form_data) {
        if (form_el.render_conditions && form_el.render_conditions.length > 0) {
          if (
            this.are_form_entry_conditions_satisfied(form_el.render_conditions)
          ) {
            ret.push(form_el)
          }
        } else {
    ...
    is_valid() {
      ...
      if (this.$refs.form_element)
        for (let form_item of this.$refs.form_element) {
          if (!form_item.is_input_valid()['is_valid']) return false
        }
