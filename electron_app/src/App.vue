<template>
  <div id="app">
    ...
    <ApplicationFrame
      v-else
      title="DiffusionBee - Stable Diffusion GUI"
...
export default {
  ...
  methods: {
    ...
    check_for_updates() {
      ...
      xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
          ...
          if (latest_app_version != current_version) {
            if (
              native_confirm(
                'A new version of ' +
    ...
    set_show_dialog_on_quit() {
      ...
      if (
        this.app_state.train.state == 'paused' ||
        this.app_state.train.state == 'training'
      ) {
        this.should_show_dialog_on_quit = true
        this.show_dialog_on_quit_msg =
          'Training has not been completed. Are you sure you want to quit?'
        window.ipcRenderer.sendSync(
          'show_dialog_on_quit',
          this.show_dialog_on_quit_msg
        )
  ...
  data() {
    let app_state = {
      ...
      show_dialog_on_quit_msg: '', // the message to show while quitting
      ...
      current_project_type: '', // the project type which is opened right now
      logs: '',
